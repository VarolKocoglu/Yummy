        ...
        AF.request("http://kasimadalan.pe.hu/yemekler/tumYemekleriGetir.php",method: .get).response { response in
            if let data = response.data {
                do{
                    ...
                    if let tempList = response.foods {
                        list = tempList
                    }
                    
                    self.foodsPresenter?.sendFoodsListToPresenter(foodsList: list)
        ...
        AF.request("http://kasimadalan.pe.hu/yemekler/tumYemekleriGetir.php",method: .get).response { response in
            if let data = response.data {
                do{
                    ...
                    if let tempList = response.foods {
                        list = tempList.filter { $0.foodName!.lowercased().contains(word.lowercased())}
                        if word.isEmpty {
                            list = tempList
                        }
                    }
